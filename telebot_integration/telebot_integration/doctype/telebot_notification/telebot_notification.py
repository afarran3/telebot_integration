...
from telebot_integration.telebot_integration.doctype.telebot_settings.telebot_settings import (
    send_to_telegram,
)

class TeleBotNotification(Document):
	...
	def validate_condition(self):
		...
		if self.condition:
			try:
				...
				frappe.throw(
					_("The Condition '{0}' is invalid").format(self.condition))
	...
	def get_dynamic_recipients(self, doc):
		"""
		if 'Dynamic Recipients' is checked means that notification will be sent
		to all 'Party' field in the DocType you created notification for if any
		'Party' field has value.
		"""
	...
	def send_a_telegram_msg(self, doc, context):
		...
		for telegram_user in recipients_telegram_user_list:
			...
			doc.party_type = frappe.get_value(
				"TeleBot User Settings", telegram_user, "party"
			)
			doc.to_party = frappe.get_value(
				"TeleBot User Settings", telegram_user, "telegram_user"
			)
	...
	def get_template(self):
		...
		def load_template(extn):
			...
			template_path = os.path.join(
				os.path.dirname(module.__file__), frappe.scrub(
					self.name) + extn
			)
...
def get_documents_for_today(notification):
    notification = frappe.get_doc("TeleBot Notification", notification)
...
def trigger_notifications(doc, method=None):
    ...
    if method == "daily":
        doc_list = frappe.get_all(
            "TeleBot Notification",
            filters={
                "event": ("in", ("Days Before", "Days After")), "enabled": 1},
        )
        for d in doc_list:
            alert = frappe.get_doc("TeleBot Notification", d.name)
...
def creat_extra_notification_log(doc):
    enl_doc = frappe.new_doc("TeleBot Notification Log")
